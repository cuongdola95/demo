...
<table class="table table-bordered table-hover">
    ...
    <tr><td><input type="checkbox" name="checkbox_phones" value="<?=$obj->getId()?>"></td>
      ...
      <input type="radio" id="switch_left_<?=$obj->getId()?>" phoneid="<?=$obj->getId()?>" name="status_<?=$obj->getId()?>" value="1" class="status-gop" <?= ($obj->gop == 1) ? 'checked' : '' ?> />
      <label class="label-for" for="switch_left_<?=$obj->getId()?>"> YES </label>
      <input type="radio" id="switch_right_<?=$obj->getId()?>" phoneid="<?=$obj->getId()?>" name="status_<?=$obj->getId()?>" value="0" class="status-gop" <?= ($obj->gop == 0) ? 'checked' : '' ?> />
      <label class="label-for" for="switch_right_<?=$obj->getId()?>"> NO </label>
